    ...
    public Single<String> authWithTwitchToken(String twitchToken, String twitchNick) {
        return pgClient.rxBegin().flatMap(tx -> {
            return tx.rxPreparedQuery(
                ...
                .flatMap(id ->
                    tx.rxPreparedQuery(
                        "insert into user_token(token, relates_to_user)" +
                            " VALUES ($1, $2)" +
                            " RETURNING token",
                        Tuple.of(UUID.randomUUID().toString(), id)
                    )
                )
                .map(res -> res.iterator().next().getString("token"))
                .flatMap(res -> tx.rxCommit().andThen(Single.just(res)));
        });
    }
}
